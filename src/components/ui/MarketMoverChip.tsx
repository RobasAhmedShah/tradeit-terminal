...
export const MarketMoverChip: React.FC<MarketMoverChipProps> = ({ stock, onPress }) => {
  ...
  return (
    ...
    >
      ...
      <Ionicons name={isPositive ? 'caret-up' : 'caret-down'} size={13} color={isPositive ? '#0ECB81' : '#F6465D'} />
      <Text className={`ml-1 text-[13px] font-semibold ${isPositive ? 'text-[#0ECB81]' : 'text-[#F6465D]'}`}>
