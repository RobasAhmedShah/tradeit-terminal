  ...
  return (
    <View className="bg-app-card rounded-xl border border-app-border p-2 flex-1">
      <View className="flex-row items-center justify-between mb-2">
        <Text className="text-app-text font-bold text-sm">Recent Trades</Text>
        <TouchableOpacity>
      ...
      <View className="flex-row justify-between mb-1.5">
        <Text className="text-app-muted text-[9px] flex-1">Price (PKR)</Text>
        <Text className="text-app-muted text-[9px] flex-1 text-center">Qty</Text>
        <Text className="text-app-muted text-[9px] flex-[0.8] text-right">Time</Text>
      </View>
      ...
      {visible.map((trade) => (
        <View key={trade.id} className="flex-row justify-between items-center py-1">
          ...
          <Text className="text-app-text text-[10px] flex-1 text-center">{trade.qty.toLocaleString()}</Text>
          <Text className="text-app-muted text-[9px] flex-[0.8] text-right">{trade.time}</Text>
        </View>
      ))}
    </View>
  );
